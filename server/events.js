...
function init(app) {
  ...
  app.use(route.get('/api/events/:id', async (ctx, id) => {
    const event = await models.event.findOne({
      where: { id },
    });
    const ret = event.toJSON();
    const players = await event.getPlayers();
    ret.players = await Promise.all(players.map(async (player) => {
    ...
    ret.games = await Promise.all(games.map(async (game) => {
      const g = game.toJSON();
  ...
  app.use(route.put('/api/events/:id/finish', async (ctx, id) => {
    const event = await models.event.findOne({
      where: { id },
    });
    if (!event) {
  ...
  app.use(route.put('/api/events/:id/reopen', async (ctx, id) => {
    if (!ctx.admin) {
      ctx.status = 401;
      return;
    }
  ...
  app.use(route.put('/api/events/:id', async (ctx, id) => {
    // const req = ctx.request.body;
    const event = await models.event.findOne({
      where: { id },
    });
  ...
  app.use(route.delete('/api/events/:id', async (ctx, id) => {
    if (!ctx.admin) {
      ctx.status = 401;
      return;
    }
  ...
  app.use(route.post('/api/events/:id/players', async (ctx, id) => {
    const req = ctx.request.body;
    const event = await models.event.findOne({
      where: { id },
    });
    ...
    const players = await Promise.all(req.users.map(async (userId) => {
  ...
  app.use(route.delete('/api/events/:eventId/players/:id', async (ctx, eventId, id) => {
  ...
  app.use(route.post('/api/events/:id/games', async (ctx, id) => {
    const req = ctx.request.body;
    const event = await models.event.findOne({
      where: { id },
    });
    ...
    ret.winners = await Promise.all(req.winners.map(async (playerId) => {
    ...
    ret.losers = await Promise.all(req.losers.map(async (playerId) => {
  ...
  app.use(route.delete('/api/events/:eventId/games/:id', async (ctx, eventId, id) => {
    const event = await models.event.findOne({
      where: { id: eventId },
    });
    if (event.finished) {
